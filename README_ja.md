...
`DSP` ページでは、入力の無音が続いた場合にDSP演算を自動的に休止する設定も行えます。
無音時間は0.1〜5.0秒の範囲を0.1秒刻みで指定出来ます。
...
ブラウザーで一時停止した動画など、音が出ていなくてもアプリケーションが無音PCMを
送り続ける場合のEffeTune演算を止めるには、`DSP` ページの
`無音時にDSPを休止` をオンにします。`無音時間` は0.1〜5.0秒の範囲を
0.1秒刻みで指定出来ます。スイッチをオフにすると既定値の `ignore` となり、
従来どおり無音入力でもDSP演算を継続します。オフの間も最後に選択した時間は
表示・保持され、次にオンにした時に再利用されます。初回の選択値は1.0秒です。

PipeTuneは、入力側のサンプリング周波数変換後のPCMを判定します。設定した無音時間の
間はプリセット全体の演算を続けるため、ディレイ、リバーブ、エフェクトが生成した音を
出力出来ます。その後、5msで出力をゼロへフェードし、DSP内部状態をリセットして、
ゼロPCMを出力しながらEffeTuneの呼び出しを停止します。ゼロではないサンプルを含む
最初のブロックで直ちに復帰し、そのブロックからDSP処理します。Vinylノイズの
シミュレーションなど、無音入力から音を生成するエフェクトも、この設定を有効にした
場合は指定時間後に停止します。

状態表示上のプリセット処理モードは維持され、DSP動作状態が別に表示されます。
無音時間中は `残響処理中`、DSP演算の停止後はLoadの百分率に代わって `休止中` と
表示されます。

設定ファイルには `PIPETUNE_DSP_IDLE_TIMEOUT=ignore`、または`100`〜`5000`の
100ms刻みの整数値として保存されます。
