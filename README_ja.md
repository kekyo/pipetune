...
PipeTuneは、選択中の物理出力をシステム音量の唯一の適用箇所として扱います。
デスクトップに表示されるPipeTuneの音量・ミュート操作は物理出力へのプロキシです。
PipeTune側で変更すると物理出力へ反映され、物理出力を外部から変更した場合は
PipeTune側の表示へ反映されます。

起動時と出力切替時は、選択された物理デバイスの現在の音量とミュート状態を
優先します。そのため、新しい出力先へ切り替えても、直前のデバイスの音量を
持ち越さず、そのデバイス自身に保存された音量を維持します。Bypassモードでは
PipeTuneのPCM経路で同じ音量を再適用しないため、減衰は物理出力で1回だけ
行われます。DSPプリセットが意図的に行うゲイン変更は、このシステム音量とは
独立しています。

読み書き可能な実効音量コントロールを提供しない物理シンクは選択対象になりません。
コントロールが利用できない場合や書き込みに失敗した場合は、別の利用可能な
シンクへフォールバックします。

## GTKコントロールウィンドウ

GTKウィンドウは、左側のセクション分けされたPipeTuneステータスを常に表示し、
右側で**Processing**、**Output**、**Rate**、**DSP**、**Advanced**の設定を
切り替えます。設定を変更すると、動作中のデーモンへ直ちにプレビュー反映されます。
単一の**Apply**ボタンは、デーモンが確認したすべての選択を一つの起動設定として
原子的に保存します。**Cancel**、Escape、またはタイトルバーの閉じるボタンは、
以前のライブ状態へ戻してからウィンドウを隠します。

常時表示されるペインでは、DSPの**Load**を現在のパーセント値を内包する、
横方向に伸縮するメーターで表示します。グラフの塗りつぶしは100%を上限としますが、
100%を超えた実測値もテキストにはそのまま表示します。

下部の**Action Log**ドロワーでは、接続、プレビュー、保存、失敗の履歴を参照できます。
PipeTune切断中は設定が読み取り専用になり、再接続後に処理を再開します。
トランザクションとロールバックの詳細は
[GTKアプリケーションの動作（英語）](pipetune-gtk/README.md)を参照してください。

## PipeTuneからどのデバイスに出力するか

前節の③は、ユーザーが明示的にデバイスを指定できます。

GTKウィンドウのOutputページにある**Preferred physical output**メニューから
...
GTKウィンドウのDSPページにある**Native backend**から
...
状態遷移とEMPTY/GAP処理の詳細は
[DSPとPipeWireのアイドル制御（英語）](pipetune/docs/dsp-idle.md)
を参照してください。

## PipeTune設定のリセット

GTKウィンドウのAdvancedページにある**Restore Defaults**は、次の状態を
