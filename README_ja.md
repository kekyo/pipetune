...
次の実線矢印は、音声が流れる順序を表します。PipeTuneがセッションの仮想的な
既定出力になるため、OSの音声設定はアプリケーションの音声を実デバイスへ直接送らず、
PipeTuneのDSPを経由させます。
...
flowchart LR
    ...
    tune["3. PipeTune<br/>仮想シンク → EffeTune DSP"]
    device["4. 音声デバイス<br/>内蔵オーディオ・USB DAC"]
    physical["5. 物理デバイス<br/>スピーカー・ヘッドホン"]

    app -->|"① 音声を送る"| os
    os -->|"② 既定ストリームを振り分ける"| tune
    tune -->|"③ 処理済み音声を送る"| device
    device -->|"④ 音を出す"| physical
```

手順③では、ユーザーが指定した音声デバイスが利用可能なら、そのデバイスへ出力します。
指定したUSBデバイスなどが外れている間はシステム既定へ自動的にフォールバックし、
...
先頭の**System default**を選ぶと、明示的な指定を破棄します。また、実際の出力先と、
