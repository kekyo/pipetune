...
通常のデスクトップ音声設定で選択した物理出力の直前へ、WirePlumberが非表示の
...
- 物理出力の選択と音量調整には従来どおりOSの音声設定を使い、PipeTuneの
  デバイスはその一覧に表示しません。
...
flowchart LR
    ...
    tune["4. 非表示PipeTuneフィルター<br/>(EffeTune DSP)"]
...
非表示フィルターノードは通常のデスクトップクライアントから参照できないため、
選択可能なデバイスとして表示されません。OSの出力選択、既定出力、アプリごとの
ルーティング、ミュート、音量は、PipeTune導入前と同じ方法で操作できます。
...
PipeTuneが異常終了した場合も、非表示フィルターノードが消えるとWirePlumberが
