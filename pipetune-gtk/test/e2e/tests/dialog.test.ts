...
describe('PipeTune GTK dialog', () => {
  ...
  it('uses expanded status-pane space for long values', async () => {
    ...
    const livePreset = await getElement('status-live-preset', 'label');
    expect((await livePreset.info()).states).toContain('showing');
    expect(await livePreset.text()).toBe(
      'e2e-preset-with-an-extraordinarily-long-descriptive-name.effetune_preset'
    );
    const initialWidth = (await livePreset.capture()).bounds.width;

    await resizeStatusPaneTo(680);

    const expandedWidth = (await livePreset.capture()).bounds.width;
