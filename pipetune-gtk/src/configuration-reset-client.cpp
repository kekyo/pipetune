...
struct ConfigurationResetClientImplementation {
  std::string executable;
  bool destroyed;
  bool pending;
  GCancellable *cancellable;

  explicit ConfigurationResetClientImplementation(std::string path)
      : executable(std::move(path)), destroyed(false), pending(false),
        cancellable(g_cancellable_new()) {}

  ~ConfigurationResetClientImplementation() {
    g_object_unref(cancellable);
  }
};

struct ConfigurationResetClient {
  std::shared_ptr<ConfigurationResetClientImplementation> implementation;
};

struct ConfigurationResetOperation {
  std::shared_ptr<ConfigurationResetClientImplementation> implementation;
  GSubprocess *process;
...
static void finishOperation(
    ConfigurationResetOperation *operation,
    ConfigurationResetClientResult result) {
...
static gboolean reportStartFailure(gpointer userData) {
  auto *operation =
      static_cast<ConfigurationResetOperation *>(userData);
  finishOperation(
      operation,
      {.success = false,
       .standardOutput = {},
...
static std::string unsuccessfulProcessMessage(GSubprocess *process) {
  if (g_subprocess_get_if_exited(process)) {
    return "PipeTune configuration reset exited with status " +
           std::to_string(g_subprocess_get_exit_status(process));
  }
  return "PipeTune configuration reset was terminated";
}
...
static void onCommunicated(GObject *source, GAsyncResult *result,
  ...
  auto completed = ConfigurationResetClientResult{
      .success = false, .standardOutput = {}, .error = {}};
  if (!communicated) {
    completed.error =
        glibErrorMessage(error,
                         "cannot communicate with PipeTune configuration "
                         "reset");
  } else {
    ...
    if (!completed.success) {
      ...
      if (completed.error.empty()) {
        completed.error =
            unsuccessfulProcessMessage(operation->process);
      }
    }
  }
...
ConfigurationResetClient *createConfigurationResetClient(
  ...
  return new ConfigurationResetClient{
      .implementation =
          std::make_shared<ConfigurationResetClientImplementation>(
              executable)};
}

void destroyConfigurationResetClient(
    ConfigurationResetClient *client) {
  if (client == nullptr) {
    return;
  }
...
bool resetConfigurationAsync(ConfigurationResetClient *client,
  ...
  const auto arguments = std::array<const gchar *, 5>{
      implementation->executable.c_str(), "config", "reset", "--yes",
      nullptr};
  auto *error = static_cast<GError *>(nullptr);
  ...
  auto *process =
      g_subprocess_newv(arguments.data(), flags, &error);
  auto *operation = new ConfigurationResetOperation{
  ...
  if (process == nullptr) {
    operation->startError =
        glibErrorMessage(error,
                         "cannot start PipeTune configuration reset");
